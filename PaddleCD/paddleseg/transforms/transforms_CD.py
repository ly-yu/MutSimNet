...
class Compose:
    ...
    def __call__(self, data):
        ...
        """
        if 'img1' not in data.keys():
            raise ValueError("`data` must include `img1` key.")
        if 'img2' not in data.keys():
            raise ValueError("`data` must include `img2` key.")
        if isinstance(data['img1'], str):
            data['img1'] = cv2.imread(data['img1']).astype('float32')
        if isinstance(data['img2'], str):
            data['img2'] = cv2.imread(data['img2']).astype('float32')
        if data['img1'] is None:
            raise ValueError('Can\'t read The image file {}!'.format(data[
                'img1']))
        if data['img2'] is None:
            raise ValueError('Can\'t read The image file {}!'.format(data[
                'img2']))
        if not isinstance(data['img1'], np.ndarray):
            raise TypeError("Image1 type is not numpy.")
        if not isinstance(data['img2'], np.ndarray):
            raise TypeError("Image2 type is not numpy.")
        if len(data['img1'].shape) != 3:
            raise ValueError('Image1 is not 3-dimensional.')
        if len(data['img2'].shape) != 3:
            raise ValueError('Image2 is not 3-dimensional.')
        if 'label' in data.keys() and isinstance(data['label'], str):
            data['label'] = np.asarray(Image.open(data['label']))

        if self.to_rgb:
            data['img1'] = cv2.cvtColor(data['img1'], cv2.COLOR_BGR2RGB)
            data['img2'] = cv2.cvtColor(data['img2'], cv2.COLOR_BGR2RGB)
        ...
        for op in self.transforms:
            data = op(data)
        data['img1'] = np.transpose(data['img1'], (2, 0, 1))
        data['img2'] = np.transpose(data['img2'], (2, 0, 1))
        return data
...
class RandomHorizontalFlip:
    ...
    def __call__(self, data):
        if random.random() < self.prob:
            data['img1'] = functional.horizontal_flip(data['img1'])
            data['img2'] = functional.horizontal_flip(data['img2'])
...
class ResizeStepScaling:
    ...
    def __call__(self, data):
        ...
        w = int(round(scale_factor * data['img1'].shape[1]))
        h = int(round(scale_factor * data['img1'].shape[0]))

        data['img1'] = functional.resize(data['img1'], (w, h), cv2.INTER_LINEAR)
        data['img2'] = functional.resize(data['img2'], (w, h), cv2.INTER_LINEAR)
        for key in data.get('gt_fields', []):
...
class Normalize:
    ...
    def __call__(self, data):
        ...
        data['img1'] = functional.normalize(data['img1'], mean, std)
        data['img2'] = functional.normalize(data['img2'], mean, std)
        return data
...
class RandomPaddingCrop:
    ...
    def __call__(self, data):
        ...
        img_height = data['img1'].shape[0]
        img_width = data['img1'].shape[1]
        ...
        if img_height == crop_height and img_width == crop_width:
            ...
            if crop_height > 0 and crop_width > 0:
                ...
                data['img1'] = data['img1'][h_off:(crop_height + h_off), w_off:(
                    w_off + crop_width), :]
                data['img2'] = data['img2'][h_off:(crop_height + h_off), w_off:(
                    w_off + crop_width), :]
                for key in data.get('gt_fields', []):
...
class RandomDistort:
    ...
    def __call__(self, data):
        ...
        data['img1'] = data['img1'].astype('uint8')
        data['img1'] = Image.fromarray(data['img1'])
        for id in range(len(ops)):
            params = params_dict[ops[id].__name__]
            prob = prob_dict[ops[id].__name__]
            params['im'] = data['img1']
            if np.random.uniform(0, 1) < prob:
                data['img1'] = ops[id](**params)
        data['img1'] = np.asarray(data['img1']).astype('float32')
        data['img2'] = data['img2'].astype('uint8')
        data['img2'] = Image.fromarray(data['img2'])
        for id in range(len(ops)):
            params = params_dict[ops[id].__name__]
            prob = prob_dict[ops[id].__name__]
            params['im'] = data['img2']
            if np.random.uniform(0, 1) < prob:
                data['img2'] = ops[id](**params)
        data['img2'] = np.asarray(data['img2']).astype('float32')
        return data
