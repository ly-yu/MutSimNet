...
class Attention(nn.Layer):
    ...
    def forward(self, x):
        ...
        # print(shape)
        x = x.transpose([0, 2, 3, 1]).reshape([shape[0], shape[2]*shape[3], shape[1]])
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        x = x.reshape([shape[0], shape[2], shape[3], self.channels]).transpose([0, 3, 1, 2])
        return x


@manager.MODELS.add_component
class UPerNetCDAttention(nn.Layer):
    """
    ...
    def forward(self, x):
        x, y = paddle.split(x, 2, axis=1)   # 还原两张图像
        feats1 = self.backbone(x)
        feats2 = self.backbone(y)
        feats = [feats1[i] - feats2[i] for i in self.backbone_indices]
...
class UPerNetHead(nn.Layer):
    def __init__(self,
        ...
        self.align_corners = align_corners
        self.attention = Attention(fpn_inplanes[-1], 512, 4)
        self.enable_auxiliary_loss = enable_auxiliary_loss
    ...
    def forward(self, inputs):
        ...
        laterals.append(self.attention(inputs[-1]))
