...
def evaluate(model,
    ...
    if print_detail:
        ...
        f1score = 2 * class_precision[-1] * class_recall[-1] / (class_precision[-1] + class_recall[-1])
        logger.info("[CD Info] IoU:%6.4f Precision:%6.4f Recall:%6.4f F1score:%6.4f" % (class_iou[-1], class_precision[-1], class_recall[-1], f1score))
    return f1score, miou, acc, class_iou, class_precision, kappa
