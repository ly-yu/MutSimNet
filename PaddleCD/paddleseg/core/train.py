...
def train(model,
    ...
    paddle.flops(model, [1, 6, 512, 512])
    ...
    while iter < iters:
        ...
        for data in loader:
            ...
            if (iter % save_interval == 0 or
                ...
                f1score, mean_iou, acc, _, _, _ = evaluate(
                    model,
                    val_dataset,
                    num_workers=num_workers,
                    precision=precision,
                ...
                if use_ema:
                    ema_f1score, ema_mean_iou, ema_acc, _, _, _ = evaluate(
                        ema_model,
                        val_dataset,
                        num_workers=num_workers,
                        precision=precision,
            ...
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                ...
                if val_dataset is not None:
                    ...
                    logger.info(
                        '[EVAL] The model with the best validation F1score ({:.4f}) was saved at iter {}.'
                        .format(best_f1score, best_model_iter))
                    if use_ema:
                        if ema_f1score > best_ema_f1score:
                            best_ema_f1score = ema_f1score
                            ...
                            paddle.save(ema_model.state_dict(),
                                        os.path.join(best_ema_model_dir,
                                                     'ema_model.pdparams'))
                        logger.info(
                            '[EVAL] The EMA model with the best validation F1score ({:.4f}) was saved at iter {}.'
                            .format(best_ema_f1score, best_ema_model_iter))

                    if use_vdl:
                        log_writer.add_scalar('Evaluate/f1score', f1score, iter)
